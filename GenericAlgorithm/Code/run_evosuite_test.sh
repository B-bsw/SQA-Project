#!/usr/bin/env bash
# Round 1: generate one EvoSuite suite from PROJECT BUGGY revision.
...
seed=${SEED:-20260918}; client_memory_mb=${EVOSUITE_CLIENT_MEMORY_MB:-2048}
...
[[ $(find "$resource_dir" -type f -name '*.java' | wc -l | tr -d ' ') -gt 0 ]] || { echo "No Java files in $resource_dir" >&2; exit 2; }

result_dir="$ga_root/Result_Round1/${project}_${bug_id}"
test_root="$ga_root/TestCode/${project}_${bug_id}"
archive_previous_run "$result_dir" "$test_root"
...
write_setup_failure() {
  ...
  jq -n --arg project "$project" --argjson bug_id "$bug_id" --arg state "$state" \
    ...
    '{schema_version:"2.0",report_type:"generation",round:1,project:$project,bug_id:$bug_id,algorithm:"STANDARD_GA",status:$state,error:$error,targets:[]}' > "$result_dir/result.json"
  jq -r '["project","bug_id","algorithm","seed","search_budget_seconds","target_class","generation_seconds","generated_test_methods","test_execution","line_coverage_percent","branch_coverage_percent","status","error"], [.project,.bug_id,.algorithm,"","","","",0,"NOT_RUN","","",.status,.error] | @csv' "$result_dir/result.json" > "$result_dir/result.csv"
  printf '%s\n' "$result_dir"; exit 1
}

echo "[$project-$bug_id] checkout and compile buggy revision" >&2
...
while IFS= read -r source; do
  ...
  target_tests="$temp_root/generated-$target_slug"; evo_report="$temp_root/report-$target_slug"
  ...
  execution=FAIL; state=generation_failed; error=$(tail -200 "$generated_output")
...
jq -s --arg created_at "$created_at" --arg project "$project" --argjson bug_id "$bug_id" \
  --arg resource_dir "${resource_dir#"$repo_root/"}" --arg test_code_dir "${test_root#"$repo_root/"}" \
  '{schema_version:"2.0",report_type:"generation",round:1,created_at:$created_at,project:$project,bug_id:$bug_id,resource_dir:$resource_dir,test_code_dir:$test_code_dir,algorithm:"STANDARD_GA",status:(if all(.[];.status=="ok") then "ok" else "failed" end),targets:.}' "$jsonl" > "$result_dir/result.json"
jq -r '["project","bug_id","algorithm","seed","search_budget_seconds","target_class","generation_seconds","generated_test_methods","test_execution","line_coverage_percent","branch_coverage_percent","status","error"], (.targets[] as $t | [.project,.bug_id,$t.algorithm,$t.seed,$t.search_budget_seconds,$t.target_class,$t.generation_seconds,$t.generated_test_methods,$t.test_execution,($t.line_coverage_percent//""),($t.branch_coverage_percent//""),$t.status,($t.error//"")]) | @csv' "$result_dir/result.json" > "$result_dir/result.csv"
